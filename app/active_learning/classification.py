...
class StatefulComponent:
    """
    Concept:
        Init members as cls-variables.
        Manipulate them with classmethods.
    Problem:
    ...
    @classmethod
    def fit(cls, texts, labels):
        X = cls.tfidf_vectorizer.fit_transform(texts)
        cls.clf.fit(X, labels)

    @classmethod
    def refit(cls, texts, labels):
        X = cls.tfidf_vectorizer.transform(texts)
        cls.clf.partial_fit(X, labels)

    @classmethod
    def rank(cls, texts):
        X = cls.tfidf_vectorizer.transform(texts)
        probs = cls.clf.predict_proba(X)
        return cls.margin_sampling(probs)

    @classmethod
    def margin_sampling(cls, y_probs: np.array):
        """
        Formula: x_m = argmin(x) P(y_1, x) - P(y_2, x)
            where y_1 = Most probable class of x, y_2 second most probable class of x
